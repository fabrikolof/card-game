    ...
    public Flux<DomainEvent> apply(Mono<RondaTerminada> rondaTerminadaMono) {
        return rondaTerminadaMono.flatMapMany((event) -> repository
                ...
                .flatMapIterable(events -> {
                    var juego = Juego.from(JuegoId.of(event.aggregateRootId()), events);//juego
                    var competidores = juego.jugadores().values().stream() //jugadores
                            .filter(jugador -> jugador.mazo().value().cantidad() > 0) //mazo
